CREATE TABLE DCFL_PAPER_INVOICE_IMPORT_TEMP (
  BILL_TYPE varchar(10) COMMENT '发票种类',
  INVOICE_CODE varchar(20) NULL COMMENT '发票代码',
  INVOICE_NUMBER varchar(20) NOT NULL COMMENT '发票号',
  BILL_MACHINE_NUM varchar(10) COMMENT '开票机号',
  INVOICE_TITLE varchar(1000) DEFAULT '' COMMENT '购方名称',
  TAX_REGISTRY_NUM varchar(100) NULL COMMENT '购方税号',
  INVOICE_OBJECT_ADDRESS_PHONE varchar(2000) NULL COMMENT '购方地址电话',
  INVOICE_OBJECT_BANK_ACCOUNT varchar(2000) NULL COMMENT '开户行+账号',
  ISSUED_TIME timestamp NULL COMMENT '开票日期',
  SUBMIT_TYPE varchar(10) NULL COMMENT '报送状态',
  SUBMIT_LOG varchar(2000) NULL COMMENT '报送日志',
  ISSUED_MONTH varchar(10) COMMENT '所属月份',
  TOTAL_AMOUNT decimal(12,2) NULL COMMENT '含税金额 订单总价合计',
  TAX_RATE decimal(12,2) COMMENT '税率',
  TAX_AMOUNT decimal(12,2) NULL COMMENT '税额 合计税额',
  PRODUCT_NAME varchar(1000) NOT NULL COMMENT '商品名称 主要商品名称',
  PRODUCT_TAX_ITEM varchar(100) NULL COMMENT '商品税目',
  INVOICE_MEMO varchar(2000) NULL COMMENT '备注',
  ISSUER varchar(20) NULL COMMENT '开票人',
  ACCOUNT_PAYEE varchar(20) NULL COMMENT '收款员',
  PRINT_TYPE varchar(10) NULL COMMENT '打印标志',
  INVOICE_INVALID_FLAG varchar(10) NULL COMMENT '发票作废标志',
  LIST_TYPE varchar(10) NULL COMMENT '清单标志',
  REPARE_TYPE varchar(10) NULL COMMENT '修复标志',
  REVIEWER varchar(20) NULL COMMENT '复核人',
  SALE_DEPARTMENT varchar(100) NULL COMMENT '销售部门',
  INVOICE_INVALID_DATE timestamp NULL COMMENT '作废时间',
  CREATED_BY int(10) DEFAULT NULL,
  CREATE_TIME timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  UPDATED_BY int(10) DEFAULT NULL,
  UPDATE_TIME timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
	PRIMARY KEY (`INVOICE_NUMBER`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='从页面导入纸质发票 临时表';

--上传excel的时候需要把开票时间用公式 =TEXT(I2,"yyyy/mm/dd hh:mm:ss")


#HISTORY
CREATE TABLE DCFL_PAPER_INVOICE_IMPORT_TEMP_HISTORY
AS
SELECT * FROM DCFL_PAPER_INVOICE_IMPORT_TEMP
WHERE 1 = 2